...
pub struct ArtifactEndpointsScreen {
    artifact_name: String,
}

impl ArtifactEndpointsScreen {
    pub fn new(artifact_name: String) -> Self {
        Self { artifact_name }
    }
}

impl Screen for ArtifactEndpointsScreen {
    ...
    fn draw(&mut self, f: &mut Frame, _context: &ScreenContext) {
        ...
        let title_block = Line::from(vec![
            ...
            Span::styled(
                format!("{} - endpoints", self.artifact_name),
        ...
        let content = Paragraph::new("Endpoints content will go here")
            .block(main_block);
